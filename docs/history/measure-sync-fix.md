...
- Added a custom `arePropsEqual` function to optimize re-renders. It intelligently checks `selectedEdge` and `hoveredEdge` to ensure that a figure's measures only re-render if the selection/hover applies to _that specific figure_. This prevents unnecessary re-renders of all figures when hovering a single edge.
