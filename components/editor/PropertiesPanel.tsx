...
export function PropertiesPanel() {
  ...
  const NAME_FONT_SIZE_OPTIONS = [
    12, 16, 20, 24, 32, 40, 48, 64, 80, 96, 128, 160,
  ];
  ...
  const renderFontSizeSelect = (
    ...
    const v = Math.round(Number.isFinite(current) ? current : 24);
    const allowed = new Set(NAME_FONT_SIZE_OPTIONS);
    return (
      <select
        data-testid={testId}
        className={
          "w-full " + inputBaseClass + " " + inputFocusClass + " !text-left"
        }
        value={String(v)}
        onChange={(e) => {
          const n = Number(e.target.value);
          const safe = Number.isFinite(n) ? Math.max(6, Math.min(256, n)) : 24;
          onPick(safe);
        }}
      >
        {!allowed.has(v) ? <option value={String(v)}>{v} px</option> : null}
        {NAME_FONT_SIZE_OPTIONS.map((s) => (
    ...
    return renderCollapsibleSection({
      ...
      children: (
        <div className="space-y-5">
          {/* Identificação: nome + fonte do nome na mesma linha. */}
          <div className="grid grid-cols-[1fr_88px] gap-3">
            <label className="space-y-1 block min-w-0">
              <span className={fieldLabelClass}>Nome do molde</span>
              <input
                data-testid="mold-name-input"
                className={
                  "w-full " + inputBaseClass + " " + inputFocusClass + " !text-left"
                }
                type="text"
            ...
            <label className="space-y-1 block min-w-0">
              <span className={fieldLabelClass}>Fonte</span>
              {renderFontSizeSelect(
                mold.nameFontSizePx ?? 24,
                (safe) =>
                  setFigures((prev) =>
                    prev.map((f) =>
                      f.id === mold.id ? { ...f, nameFontSizePx: safe } : f
                    )
                  ),
                "mold-name-font-size"
              )}
          ...
          {/* Especificações de corte: pares lado a lado (valores curtos). */}
          <div className="space-y-3">
            <div className={subgroupHeaderClass}>Especificações de corte</div>
            <div className="grid grid-cols-2 gap-3">
              <label className="space-y-1 block min-w-0">
                <span className={fieldLabelClass}>Tamanho base</span>
                <input
                  data-testid="mold-base-size"
                  className={"w-full " + inputBaseClass + " " + inputFocusClass}
                  value={mold.moldMeta?.baseSize ?? ""}
                  onChange={(evt) => {
                    ...
                    setFigures((prev) =>
                      prev.map((f) =>
                        f.id === mold.id
                          ? {
                              ...f,
                              moldMeta: { ...(f.moldMeta ?? {}), baseSize: value },
                            }
                          : f
                      )
                    );
              ...
              <label className="space-y-1 block min-w-0">
                <span className={fieldLabelClass}>Quantidade</span>
                <input
                  data-testid="mold-quantity"
                  className={"w-full " + inputBaseClass + " " + inputFocusClass}
                  type="number"
                  min={1}
                  value={String(mold.moldMeta?.cutQuantity ?? 1)}
                  onChange={(evt) => {
              ...
              <label className="space-y-1 block min-w-0">
                <span className={fieldLabelClass}>Fio (seta)</span>
                <select
                  data-testid="mold-grainline-enabled"
                  className={
                    "w-full " +
                    inputBaseClass +
                    " " +
                  ...
                  value={
                    typeof mold.moldMeta?.grainline?.angleDeg === "number"
                      ? "sim"
                      : "nao"
                  }
                  onChange={(evt) => {
                    const enabled = evt.target.value === "sim";
                    setFigures((prev) =>
                      prev.map((f) => {
                        if (f.id !== mold.id) return f;
                        if (!enabled) {
                          const meta = { ...(f.moldMeta ?? {}) };
                          delete meta.grainline;
                          return { ...f, moldMeta: meta };
                        }
                        if (
                          typeof f.moldMeta?.grainline?.angleDeg === "number"
                        ) {
                          return f;
                        }
                        // Same heuristic as mold extraction: vertical arrow
                        // for wide pieces, horizontal for tall ones. Position,
                        // direction and length are then adjusted via the
                        // arrow's handle / inner transform.
                        const bb = figureWorldBoundingBox(f);
                        const angleDeg = bb && bb.width >= bb.height ? 0 : 90;
                        return {
                          ...f,
                          moldMeta: {
                            ...(f.moldMeta ?? {}),
                            grainline: { angleDeg, autoGenerated: true },
                          },
                        };
                      })
                    );
              ...
              <label className="space-y-1 block min-w-0">
                <span className={fieldLabelClass}>Cortar na dobra</span>
                <select
                  data-testid="mold-cut-on-fold"
                  className={
                    "w-full " +
                    inputBaseClass +
                    " " +
                  ...
                  value={mold.moldMeta?.cutOnFold === true ? "sim" : "nao"}
                  onChange={(evt) => {
                    const checked = evt.target.value === "sim";
                    setFigures((prev) =>
                      prev.map((f) =>
                        f.id === mold.id
                          ? {
          ...
          {/* Tamanho do texto (linhas de documentação) — acima de Notas. */}
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2">
              <span className={fieldLabelClass}>Tamanho do texto</span>
              {renderFontSizeSelect(
                mold.moldMeta?.docFontSizePx ?? 14,
                (safe) =>
                  setFigures((prev) =>
                    prev.map((f) =>
                      f.id === mold.id
                ...
                setFigures((prev) =>
                  prev.map((f) =>
                    f.id === mold.id
                      ? {
                          ...f,
                          moldMeta: { ...(f.moldMeta ?? {}), notes: value },
                        }
                      : f
                  )
                );
          ...
          <p className="text-[10px] text-gray-400 dark:text-gray-500 leading-snug">
            ...
            mostrado como uma seta. Arraste os handles para posicionar; duplo
            clique em um handle abre o redimensionar/rotacionar.
          </p>
        </div>
      ),
    });
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-3">
                    <div className="space-y-2">
                      ...
                      {renderFontSizeSelect(
                        selectedFigure.nameFontSizePx ?? 24,
                        (safe) =>
                          setFigures((prev) =>
                            prev.map((f) =>
                              f.id === selectedFigure.id
