...
function isFigureEligibleForMoldConversion(figure: Figure): boolean {
  if (figure.kind === "seam") return false;
  if (figure.kind === "mold") return false;
  if (figure.tool === "text") return false;
  return figure.edges.length > 0;
...
export default function Canvas() {
  ...
  const handleContextConvertMoldToFigure = useCallback(() => {
    if (!edgeContextMenu) return;
    const figId = edgeContextMenu.figureId;
    const fig = figures.find((f) => f.id === figId) ?? null;
    ...
    setFigures((prev) => {
      let changed = false;
      const next = prev
        .filter((f) => !(f.kind === "seam" && f.parentId === figId))
        .map((f) => {
    ...
    setSelectedEdge(null);
    setTool("select");
    toast("Molde convertido em figura.", "success");
    setEdgeContextMenu(null);
  }, [
    edgeContextMenu,
    figures,
  ...
  const handleContextConvertFigureToMold = useCallback(() => {
    if (!edgeContextMenu) return;
    const figId = edgeContextMenu.figureId;
    const source = figures.find((f) => f.id === figId) ?? null;
    if (!source || !isFigureEligibleForMoldConversion(source)) {
      setEdgeContextMenu(null);
      return;
    }

    if (!source.closed && !hasClosedLoop(source)) {
      toast("A figura precisa estar fechada para converter em molde.", "error");
      setEdgeContextMenu(null);
      return;
    }

    const minX = Math.min(...source.nodes.map((n) => n.x));
    const minY = Math.min(...source.nodes.map((n) => n.y));
    const maxX = Math.max(...source.nodes.map((n) => n.x));
    const maxY = Math.max(...source.nodes.map((n) => n.y));
    const width = maxX - minX;
    const height = maxY - minY;
    const grainAngleDeg = width >= height ? 0 : 90;
    const fallbackName = `Molde ${moldCount + 1}`;
    ...
    setFigures((prev) =>
      prev.map((f) => {
        ...
        return {
          ...
          kind: "mold",
          closed: f.closed || hasClosedLoop(f),
          stroke: "aci7",
          fill: "rgba(96,165,250,0.22)",
          moldMeta: {
    ...
    setSelectedEdge(null);
    setTool("select");
    toast("Figura convertida em molde.", "success");
    setEdgeContextMenu(null);
  }, [
    edgeContextMenu,
    figures,
  ...
  const handlePointerDown = (
    ...
    if (tool === "pique" && e.evt.button === 0) {
      ...
      const hoveredOrHitPique =
        hoveredPique ?? findHoveredPique(figures, world, 8 / scale);
      if (hoveredOrHitPique) {
        const { figureId, piqueId } = hoveredOrHitPique;
        setFigures((prev) =>
          prev.map((f) => {
            if (f.id !== figureId) return f;
      ...
      >
            ...
            {(() => {
              ...
              return (
                <>
                  {hasFigureActions ? (
                    <>
                      {edgeContextMenu.edgeKind ? (
                        <div className="my-1 h-px bg-gray-200 dark:bg-gray-700" />
                      ) : null}
                      {canConvertMoldToFigure ? (
                        <button
                          type="button"
                          data-testid="edge-context-convert-mold-to-figure"
                          className="w-full text-left text-xs px-3 py-2 rounded hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-900 dark:text-white"
                          onClick={handleContextConvertMoldToFigure}
                        >
                          Converter em figura
                        </button>
                      ) : null}
                      {canConvertFigureToMold ? (
                        <button
                          type="button"
                          data-testid="edge-context-convert-figure-to-mold"
                          className="w-full text-left text-xs px-3 py-2 rounded hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-900 dark:text-white"
                  ...
                  {fig.mirrorLink ? (
                    <>
                      {edgeContextMenu.edgeKind || hasFigureActions ? (
                        <div className="my-1 h-px bg-gray-200 dark:bg-gray-700" />
                      ) : null}
                      <button
                        type="button"
        ...
        <Stage
          ...
          onContextMenu={(e) => {
            ...
            let fig = edgePick?.figure ?? null;
            let edge = edgePick?.edge ?? null;
            ...
            if (!fig) {
              ...
              fig = pickedId
                ? (figures.find((f) => f.id === pickedId) ?? null)
                : null;
              edge = null;
            }
