...
export function UnitSettings() {
  ...
  useEffect(() => {
    if (pixelsPerUnit === 1) {
      if (unit === 'cm') setPixelsPerUnit(37.8); // approx 96 DPI / 2.54
    ...
    <div className="relative" ref={menuRef}>
      ...
      {isOpen && (
        <div className="absolute top-full right-0 mt-1 w-64 bg-white dark:bg-surface-dark border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg p-4 z-50">
          <h3 className="text-sm font-semibold mb-3 text-gray-900 dark:text-white">Configurações de Régua</h3>
          
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <label className="text-xs text-gray-600 dark:text-gray-300">Mostrar Réguas</label>
              <button 
                onClick={() => setShowRulers(!showRulers)}
                className={`w-10 h-5 rounded-full relative transition-colors ${showRulers ? 'bg-primary' : 'bg-gray-300 dark:bg-gray-600'}`}
              >
                <span className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-transform ${showRulers ? 'left-6' : 'left-1'}`} />
              </button>
            </div>

            <div>
              <label className="block text-xs text-gray-600 dark:text-gray-300 mb-1">Unidade de Medida</label>
              <select 
                value={unit} 
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <input 
                  ...
                  className="w-full text-sm border border-gray-300 dark:border-gray-600 rounded px-2 py-1.5 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-primary"
                />
                <span className="text-xs text-gray-500 dark:text-gray-400 whitespace-nowrap">px / {unit}</span>
              </div>
