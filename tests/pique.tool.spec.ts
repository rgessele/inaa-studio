...
test("pique: funciona quando a geometria tem loop fechado mesmo com flag closed=false", async ({
  page,
}) => {
  await gotoEditor(page);
    ...
    .poll(async () => {
      return await page.evaluate(() => {
        const figs = (window.__INAA_DEBUG__?.getFiguresSnapshot?.() ??
          []) as unknown;
        const list = Array.isArray(figs) ? (figs as FigureSnapshot[]) : [];
        const base = list.find((f) => f.id === "fig_loop_closed_false") ?? null;
        return base?.piques?.length ?? 0;
      });
    })
    .toBe(1);
  ...
  await expect
    .poll(async () => {
      return await page.evaluate(() => {
        return window.__INAA_DEBUG__?.countStageNodesByName?.("inaa-pique") ?? 0;
      });
    })
    .toBeGreaterThan(0);
});
